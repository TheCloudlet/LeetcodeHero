...
```text
┌──────────────┐     ┌──────────────┐     ┌──────────────┐     ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│  PyTorch /   │────►│   MLIR IR    │────►│   Dialect    │────►│   Lowering   │────►│   Backend    │────►│ NPU Model    │
│  TensorFlow  │     │ (High-Level) │     │  Conversions │     │  (LLVM/SPIR/ │     │  Compiler    │     │    Binary    │
│              │     │              │     │              │     │   Custom)    │     │              │     │  (model.bin) │
└──────────────┘     └──────────────┘     └──────────────┘     └──────────────┘     └──────────────┘     └──────────────┘
  Input: Model        Comp Graph IR       Target-Specific      Low-Level IR/         NPU Instructions      Output: Compiled
  Tensor Graph        tensor/linalg       tpu/gpu/npu          llvm/spirv/custom     + Weights/Metadata    Model Artifact
                                          dialects             ISA generation        Optimization
...
## 2. Target: NPU (Neural Processing Unit)
...
### 2.1 NPU Architecture & Execution Model
...
## 3. Compiler Architecture: T-Diagram Representation
...
### 4.2 Answer C: MLIR's Role Across Compiler Stages
...
### 4.3 Answer A & B: Two Compilation Paths
...
## 5. Multi-Target Strategy: Supporting Multiple NPU Architectures

### 5.1 The Problem: Multiple Target Architectures
...
### 5.6 Answer Summary: Where to Modularize?
...
## 6. Lowering 實作詳解：Pattern Rewriting vs Instruction Selection

### 6.1 核心概念：Lowering 不是 1-1 Mapping
...
### 6.2 MLIR vs LLVM: 為什麼不需要 Cost Model？
...
### 6.3 Lowering 實作方式 1: Simple 1-to-1 Conversion
...
### 6.4 Lowering 實作方式 2: 1-to-N Decomposition
...
### 6.5 Lowering 實作方式 3: N-to-1 Fusion
